...
std::shared_ptr<VertexArrayObject> BuildPointCloudVAO(const PointCloud* pointCloud, const Program* pointProg)
{
    ...
    // TODO: use a VBO because data is static.
...
void OldRender(const Program* pointProg, const Texture* pointTex, const PointCloud* pointCloud, const glm::mat4& cameraMat)
{
    SDL_GL_MakeCurrent(window, gl_context);
...
static std::vector<glm::vec3> s_positionVec;
static std::vector<glm::vec2> s_uvVec;
static std::vector<glm::vec4> s_colorVec;
static std::vector<uint32_t> s_indexVec;

void OldRender2(const Program* pointProg, const Texture* pointTex, const PointCloud* pointCloud, const glm::mat4& cameraMat)
{
    SDL_GL_MakeCurrent(window, gl_context);
    ...
    // lazy init of attribute vectors
    if (s_positionVec.size() == 0)
    {
        s_positionVec.reserve(pointCloud->GetPointVec().size() * NUM_CORNERS);
        s_uvVec.reserve(pointCloud->GetPointVec().size() * NUM_CORNERS);
        s_colorVec.reserve(pointCloud->GetPointVec().size() * NUM_CORNERS);

        for (auto&& p : pointCloud->GetPointVec())
        {
            s_positionVec.push_back(glm::vec3(p.position[0], p.position[1], p.position[2]));
            s_positionVec.push_back(glm::vec3(p.position[0], p.position[1], p.position[2]));
            s_positionVec.push_back(glm::vec3(p.position[0], p.position[1], p.position[2]));
            s_positionVec.push_back(glm::vec3(p.position[0], p.position[1], p.position[2]));
            s_uvVec.push_back(uvLowerLeft);
            s_uvVec.push_back(glm::vec2(uvUpperRight.x, uvLowerLeft.y));
            s_uvVec.push_back(uvUpperRight);
            s_uvVec.push_back(glm::vec2(uvLowerLeft.x, uvUpperRight.y));
            glm::vec4 color(p.color[0] / 255.0f, p.color[1] / 255.0f, p.color[2] / 255.0f, 1.0f);
            s_colorVec.push_back(color);
            s_colorVec.push_back(color);
            s_colorVec.push_back(color);
            s_colorVec.push_back(color);
        }
    ...
    pointProg->SetAttrib("position", s_positionVec.data());
    pointProg->SetAttrib("uv", s_uvVec.data());
    pointProg->SetAttrib("color", s_colorVec.data());

    const size_t NUM_INDICES = 6;

    // lazy init of index vec
    if (s_indexVec.size() == 0)
    {
        s_indexVec.reserve(pointCloud->GetPointVec().size() * NUM_INDICES);
        assert(pointCloud->GetPointVec().size() * 6 <= std::numeric_limits<uint32_t>::max());
        for (uint32_t i = 0; i < (uint32_t)pointCloud->GetPointVec().size(); i++)
        {
            s_indexVec.push_back(i * NUM_CORNERS + 0);
            s_indexVec.push_back(i * NUM_CORNERS + 1);
            s_indexVec.push_back(i * NUM_CORNERS + 2);
            s_indexVec.push_back(i * NUM_CORNERS + 0);
            s_indexVec.push_back(i * NUM_CORNERS + 2);
            s_indexVec.push_back(i * NUM_CORNERS + 3);
        }
    }

    glDrawElements(GL_TRIANGLES, (GLsizei)(NUM_INDICES * pointCloud->GetPointVec().size()), GL_UNSIGNED_INT, s_indexVec.data());
